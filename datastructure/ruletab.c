...
RuleTab ruletabcreate(int n_max) {
    return buffer_create(n_max * sizeof(Rule*));
}
...
void ruletabfree(RuleTab* ruletab) {
    for (RuleId id = 0; id < ruletab -> n; id++) {
        free_rule(*((ruletab -> mem) + (id * sizeof(Rule*))));
    }
    buffer_free(ruletab);
}
...
Rule* ruletabadd(RuleTab* ruletab, Rule* rule) {
    Rule** is_inserted = buffer_add(ruletab, rule, sizeof(Rule*));
    if (is_inserted != NULL) {
...
Rule* ruletabaddrealoc(RuleTab* ruletab, Rule* rule) {
    buffer_add_realoc(ruletab, rule, sizeof(Rule*));
    rule -> id = ruletab -> n; // Pour que la rêgle conaisse son ID
    return rule;
}
...
Rule* ruletabget(RuleTab* ruletab, RuleId id) {
    printf("read %p, buffer%p, buffer+%ld\n", (ruletab -> mem) + (id * sizeof(Rule*)), ruletab -> mem, (id * sizeof(Rule*)));
    return *((ruletab -> mem) + (int) (id * sizeof(Rule*)));
}
